...
public class ListeErreur {
    ...
    /**
     * R�cup�re les erreurs se trouvant dans le rapport XML Baton.
     *
    ...
    /**
     * R�cup�re le node enfant via son nom.
     *
     * @param list Le node o� on doit chercher.
     * @param name Nom du node � trouver.
     * @return Retourne le node demand�, sinon retourne <code>null</code> s'il
     * ne trouve pas.
     */
    ...
    /**
     * Ajouter les erreurs se trouvant dans ce node (du rapport XML Baton) � la
     * liste d'erreur.
     *
     * @param nodeList Le node contenant la liste des erreurs � ajoute aux
     * erreurs.
     */
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            if (nodeList.item(i).getNodeType() == Node.ELEMENT_NODE) {
                ...
                switch (nodeList.item(i).getNodeName()) {
                    ...
                    // Quand on tient une des erreurs, on l'ajoute � la liste:
                    case "error":
                        ...
                        if (item.equals("Defective Pixel")) {
                            ...
                            // R�cup�re les positions du/des pixels:
                            Node param = ((Element) ((Element) nodeList.item(i)).getElementsByTagName("Params").item(0)).getElementsByTagName("Param").item(0);
                            String coordonnees = ((Element) param).getAttribute("Value"); // R�cup�re le 1er car il n'y en a qu'un...
    ...
    /**
     * Retourne la liste d'erreur avec un seule type d'erreur, re�u en
     * param�tre.
     *
     * @param type Le filtre des erreurs qu'on souhaite retourner.
     * @return La liste d'erreur avec juste celle avec le type d'erreur re�u en
     * param�tre.
     */
    ...
    /**
     * Afficher l'ensemble des types d'erreurs rencontr� dans le rapport.
     */
